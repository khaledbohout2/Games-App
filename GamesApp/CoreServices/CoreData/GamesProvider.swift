...
//    }
    ...
    private func importQuakes(from propertiesList: [Game]) {
    ...
    private func newBatchInsertRequest(with propertyList: [Game]) -> NSManagedObjectContext? {
        let context = newTaskContext()
        var index = 0
        let total = propertyList.count
        guard index < total else { return nil }
        for _ in propertyList {
            do {
...
//        let batchInsertRequest = NSBatchInsertRequest(entity: Game.entity(), dictionaryHandler: { dictionary in
